...
\begin{remark}
    \doubt{why if we impose an orderin on sets \([n]\) so that we can omit having to work with n and m. we would  simply say \([m]\leq [n]\) to mean \(m\leq n\).}
    ...
    \red{CUIDADO}. Sí puede haber una función que preserve el orden de [m] a [n] incluso si m > n. Simplemente hacemos que no sea inyectiva.
\end{remark}
