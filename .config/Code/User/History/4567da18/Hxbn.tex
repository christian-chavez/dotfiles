...
a^2+b^2=c^2
\]
it is called primitive if  \(\operatorname{gcd}(a, b, c)=1\). (\textit{Definition.} A common divisor of nonzero integers \(a_1, a_2, \ldots, a_n\) is an integer \(c\) such that \(c \mid a_i\) for all \(i\in \left\{ 1,\dots,n \right\}\); the largest of the common divisors, denoted by \(\left(a_1, a_2, \ldots, a_n\right)\), is called the greatest common divisor.)
\begin{enumerate}[label=(\alph*)]
