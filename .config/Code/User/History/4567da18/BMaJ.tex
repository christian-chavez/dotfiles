...
\begin{solution}
\begin{enumerate}[label=(\alph*)]
    \item Note \(|z^2| = |zz|=|z|^2\).
    Now \(z^2=\left(q^2-p^2\right)+i 2 q p\), so that \(\left|z^2\right|=\left(q^2-p^2\right)^2+(2 q p)^2\). On the other hand, \(|z|^2=\left(q^2+p^2\right)^2\). Thus, if we define \(a=q^2-p^2\), \(b=2 q p\), and \(c=q^2+p^2\), then \(a^2+b^2=c^2\) and \((a, b, c)\) is a Pythagorean triple.
\end{enumerate}
\end{solution}
