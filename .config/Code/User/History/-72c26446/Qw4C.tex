...
\begin{theorem}[ \(\simeq\) is an equivalence relation]\label{homotopy.is.eq.relation}
...
\begin{proof} Notice this does not follow from Theorem \ref{homotopy.is.eq.relation} as path homotopy is a stronger notion than homotopy alone. Nevertheless, we only need to modify the proof to verify that path homotopies fix the endpoints of paths.
\begin{enumerate}[label=(\roman*)]
...
\begin{theorem}[Multiplication of path homotopy classes is an operation on \(\Omega_1(X,x_0)  / \!\!\sim\)]
    For any topological space \(X\) and any \(x_0\in X\), the set \(\Omega_1(X,x_0)  / \!\!\sim\) is a group endowed with the operation  of  multiplication of path homotopy classes.
\end{theorem}
