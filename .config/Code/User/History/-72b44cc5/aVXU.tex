...
\begin{definition}[Isomorphism]
    Let \(\mathbf{C}\) be a category. A \(\mathbf{C}\)-morphism \(f\colon A\to B\) is an \textit{isomorphism} if there is a \(\mathbf{C}\)-object  \(g\colon B\to A\) such that \[f\circ g = 1_B\quad\text{and}\quad g\circ f = 1_A.\]
\end{definition}
