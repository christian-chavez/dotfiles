...
\begin{enumerate}[label=\arabic*.]
    ...
    \item \textbf{The opposite category.} If \(\mathbf{C}\) is any category, we can build a new category named the opposite of \(\mathbf{C}\) and denoted \(\mathbf{C}^{\text{op}}\). This category has the same objects as \(\mathbf{C}\), but whenever \(f\colon \)
\end{enumerate}
\end{example}
