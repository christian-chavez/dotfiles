...
Sometimes, what we have defined as functor is called \textit{covariant functor}. We may use these terms  interchangeably. Thus, when we say \textit{functor} only, we mean \textit{covariant} functor . The aditional adjetive is used in order to distinguish from contravariant functors which we now define.
